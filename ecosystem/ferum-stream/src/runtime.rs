...
use crate::metrics;
use crate::convert::{convert_ferum_event, FerumEvent};
...
pub fn bootstrap(
    ...
    let runtime = Builder::new_multi_thread()
        .thread_name("ferum-stream")
        .enable_all()
        .build()
        .expect("[ferum-stream] failed to create runtime");
    ...
    runtime.spawn(async move {
        ...
        let mut streamer = FerumStreamer::new(
            context_arc,
            starting_block,
    ...
    pub async fn convert_next_block(&mut self) -> Vec<FerumEvent> {
            ...
            .get_block_info_by_height(self.current_block_height)
        {
            ...
            Err(err) => {
                ...
                warn!(
                    "[ferum-stream] failed to get block info for block_height={}. Error: {}",
        ...
        let transactions = match self.context.get_transactions(
            ...
            Err(err) => {
                error!("[ferum-stream] failed to get transactions: {}", err);
        ...
        if transactions.is_empty() {
            debug!("[ferum-stream] no transactions to send");
            sleep(Duration::from_millis(100)).await;
            return vec![];
        }
        debug!(
            "[ferum-stream] got {} transactions from {} to {} [version on last actual transaction {}]",
            transactions.len(),
            block_start_version,
            block_last_version,
